...
class CompetitionTask:
    def __init__(self, mode='visitor'):
        ...
        self.obstacles_spawned = False  # 防止重复生成障碍物的标志位
    ...
    def do_visitor_task(self):
        # ===== 启动官方环境生成障碍物 =====
        if not getattr(self, 'obstacles_spawned', False):
            rospy.loginfo("[Task1] Starting official simulator to spawn obstacles...")
            subprocess.Popen("cd /home/kongzhu/bobac3_ws && ./reinovo_bobac3_sim", shell=True)
            time.sleep(3)
    ...
    def do_admin_task(self):
        ...
        # -------- 阶段1.5：启动官方环境生成障碍物 --------
        if not getattr(self, 'obstacles_spawned', False):
            rospy.loginfo("[Task2] Starting official simulator to spawn obstacles...")
            subprocess.Popen("cd /home/kongzhu/bobac3_ws && ./reinovo_bobac3_sim", shell=True)
            time.sleep(3) # 给予启动和生成障碍物的时间
            self.obstacles_spawned = True
    ...
    def _navigate_with_reverse(self, x, y, yaw, timeout=NAV_TIMEOUT):
        ...
        while not rospy.is_shutdown():
            ...
            if finished and client.get_state() == actionlib.GoalStatus.SUCCEEDED:
                ...
                rospy.logwarn("[Nav] 路径被阻挡或导航失败，等待 0.1 秒后立刻重新规划...")
                client.cancel_goal()
                rospy.sleep(0.1)
